...
> **Version**: Phase 2 Complete (Structured DB + Vector Search)
...
| Embedding | Voyage AI (voyage-4) | 텍스트 → 512차원 벡터 |
...
```
uesp.db (SQLite)
├── pages           — 30,506 raw wikitext pages
...
├── lore_chunks     — 8,997 lore text chunks
...
| 크롤링 페이지 | 6,731 |
| 생성된 청크 | 8,997 |
| 평균 청크 크기 | 515.8 tokens |
| 총 토큰 수 | 4,640,321 (약 464만) |
| 임베딩 완료 | 8,997 / 8,997 (100%) |
| LanceDB 벡터 | 8,997개 (512차원, float32) |
...
|---------|-------|
| Model | voyage-4 |
| Output Dimension | 512 (Matryoshka) |
...
|-----------|----------|------|
| Embedding (voyage-4) | Voyage AI | **$0** (Free tier: 200M tokens, used ~5M = 2.5%) |
